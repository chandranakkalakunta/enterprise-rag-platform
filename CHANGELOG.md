...
- Coordinator: `enable_edge=true` apply + DNS + IAP test users

### Added
- **Phase 6.1:** Terraform edge stack (`enable_edge`) — global HTTPS LB, serverless NEGs, IAP backends, path routing, Cloud Run ingress lock for web/api, IAP→`run.invoker` IAM; [edge-lb-iap runbook](docs/runbooks/edge-lb-iap.md)
- **Phase 6.0:** [ADR-0012](docs/adr/0012-production-edge-lb-iap.md) — production browser access via **HTTPS LB + IAP + serverless NEG**; **no public `run.invoker`**
